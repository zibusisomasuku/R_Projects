...
ata_function = function(my_data){
  ...
  round(c(attr(ata(cum_tri), "vwtd")), 5)
}

ata_fy19 = lapply(fy2019_gold_splits, FUN = ata_function)
write.list(ata_fy19, "./output/ata_fy19_gold.csv", t.name = names(ata_fy19))
...
#Save to CSV
write.csv(summarized_gold_index, file = "./output/summarized_index.csv")
